...
const foldersHandler = async (event: APIGatewayEvent, context: Context) => {
  ...
  switch (httpMethod as HttpMethods) {
    case 'POST': {
      ...
      const { data: countedFolders } = await faunaDBClient.query<{ data: number }>(
        Count(Paginate(Match(
      ...
      if (countedFolders[0] >= MAX_FOLDERS_PER_USER) {
        return createErrorResponse({ message: 'Max amount of folders per user exceeded' })
      }
