...
describe('addFolder', () => {
  ...
  describe('data validation', () => {
    it('missing name', async () => {
      const requestData: AddFolderRequest = {
        description: 'test2',
      } as AddFolderRequest
    ...
    it('invalid name', async () => {
      const requestData: AddFolderRequest = {
        description: 'test2',
        name: { test: true },
      } as unknown as AddFolderRequest
      const request = createRequest(requestData, {
        httpMethod: 'POST',
        headers: createAuthHeaders(loggedUserToken),
      })
    ...
    it('when all data is valid', async () => {
      ...
      await lambdaTester(getUserFoldersHandler)
        ...
        .expectResult((result: LambdaResponse) => {
          ...
          const isAdded = responseBody.folders.findIndex(
            (folder) => folder.id === addedFolderId,
          )
