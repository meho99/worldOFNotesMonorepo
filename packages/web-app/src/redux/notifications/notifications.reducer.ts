...
const notificationsPriority = [NotificationTypes.Success, NotificationTypes.Error]

const checkPriority = (notification: NotificationData) =>
  notificationsPriority.indexOf(notification.type)

export const sortNotifications = (a: NotificationData, b: NotificationData) =>
...
const notificationsSlice = createSlice({
  ...
  reducers: {
    ...
    addSuccessNotification: (
      state,
      { payload = 'Success!' }: PayloadAction<string | undefined>,
    ) => {
      notificationsAdapter.addOne(state, {
        id: generateId(),
        message: payload,
